...
custom_colormaps = {
    "ACE": colormaps.ColormapOptions(colormap="inferno", normalize=False, colormap_min=0, colormap_max=1),
    "depth_59": colormaps.ColormapOptions(colormap="gray", normalize=True, colormap_min=0, colormap_max=1, invert=True),
    "depth": colormaps.ColormapOptions(colormap="gray", normalize=True, colormap_min=0, colormap_max=1, invert=True),
    "Falsecolor": colormaps.ColormapOptions()
}
                    ...
                    if is_depth:
                        output_image = (
                            colormaps.apply_depth_colormap(
                                ...
                                colormap_options=custom_colormaps[rendered_output_name]
                            )
                            .cpu()
                            .numpy()
                        )
...
class RenderCameraPath(BaseRender):
    ...
    def main(self) -> None:
        ...
        #     test_mode="inference",
        # )
        config, pipeline, _, _ = hsi_eval_setup(
            self.load_config,
            eval_num_rays_per_chunk=self.eval_num_rays_per_chunk,
            test_mode="inference",
